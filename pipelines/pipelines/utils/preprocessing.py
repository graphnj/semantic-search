...
def convert_files_to_dicts_pmy(dir_path: str,
                           split_method: str,
                           clean_func: Optional[Callable] = None,
                           split_answers: bool = False,
                           encoding: Optional[str] = None) -> List[dict]:
            ...
            if split_method=="paragraphs":
                ...
                if len(paragraphs) > 0:
                    for index in range(len(paragraphs)):
                        if (split_answers):
                            query, answer = paragraphs[index].split('\t')
                            documents.append({
                                "content": query,
                                "meta": {
                                    "name": path.name,
                            ...
                            if index == 0 and len(paragraphs) >= 2:
                                documents.append({
                                    ...
                                    "context": paragraphs[index] + "</br>" + paragraphs[index + 1],
                                    "meta": {
                                        "name": path.name
                                    }
                                })
                            elif index == len(paragraphs)-1 and len(paragraphs) >= 2:
                                documents.append({
                                    "content": paragraphs[index],
                                    "context": paragraphs[index - 1] + "</br>" + paragraphs[index],
                                    "meta": {
                                        "name": path.name
                                    }
                                })
                            elif index ==len(paragraphs)-1 and len(paragraphs)==1:
                                documents.append({
                                    "content": paragraphs[index],
                                ...
                                documents.append({
                                    "content":  paragraphs[index],
                                    "context": paragraphs[index - 1] + "</br>" + paragraphs[index] + "</br>" + paragraphs[index + 1],
                                    "meta": {
                                        "name": path.name
                                    }
                                })
                ...
                for para in text.split("\n"):
                    ...
                    for sentence in para.split("。"):
                        if not sentence.strip():  # skip empty paragraphs
                            continue
